  ...
  return {
    ...
    developmentSuggestions: {
      ...
      supplementarySuggestions: {
        targetedTraining: `**这是你当前阶段成长速度最快的路径——不是上课，是用输出倒逼能力生长。**你需要的不只是知识输入，而是在真实压力下被验证过的能力。建议瞄准这三个方向：（1）战略思维工作坊——结业标准不是听完了课，而是亲手拆解了一个你完全陌生的行业，交出一份能让父辈和老臣认真看的判断报告；（2）资本运作实战营——以完成一个微型并购模拟或独立做一场融资路演为硬指标，让你第一次真正"摸到钱的感觉"；（3）数据决策训练——不是学工具，而是用你自己的项目数据跑出一个让团队信服的决策依据。每一项的终点都是输出，不是听懂。`,

        talentIncubator: `**传统就业路径对你来说太慢了——你需要的是一个专为你这类人设计的"成长加速器"。**定制孵化器的价值不在于给你一份工作，而在于用2-3年时间同时解决你两个核心问题：一是在不同行业、不同职能的实战轮转中，亲手画出自己的"方向地图"（不再靠外部输入判断往哪走）；二是在导师团（战略/资本/运营多领域导师陪跑）和同侪网络（跟你处于同样阶段的二代群体）中，找到第一批"同路人"。最终在导师护航下独立操盘一个从0到1的商业项目——这个项目的成功或失败，都比你做三年边缘岗位学到的多十倍。`,
      },
